...
func mustRegisterOrGet[T prometheus.Collector](reg prometheus.Registerer, c T) T {
	if err := reg.Register(c); err != nil {
		...
		// Fall through and still return c so callers do not panic on register races
		// with incompatible collectors; metrics may be silent in that edge case.
	}
	return c
}
...
func sqlOp(sql string) string {
	...
	case "select", "insert", "update", "delete",
		...
		return op
	default:
		// xorm may emit "BEGIN TRANSACTION"
